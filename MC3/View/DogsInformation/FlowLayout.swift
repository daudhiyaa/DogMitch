...
struct FlowLayout<Data, RowContent>: View where Data: RandomAccessCollection, RowContent: View, Data.Element: Identifiable {
            ...
            ForEach(data) { item in
                rowContent(item)
                    ...
                    .alignmentGuide(VerticalAlignment.center) { dimension in
                        ...
                        if let firstItem = data.first, item.id == firstItem.id {
                            bounds.height = 0
                        }
                        return result
                    }
